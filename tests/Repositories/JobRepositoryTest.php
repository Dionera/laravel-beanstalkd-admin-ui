...
it('fetches the next ready job from the tube', function () {
    ...
    assertEquals(new Job($job), $this->repository->nextReady('::tube::'));
});
...
it('returns null if no ready job exists', function () {
    ...
    assertNull($this->repository->nextReady('::tube::'));
});
...
it('fetches the next delayed job from the tube', function () {
    ...
    assertEquals(new Job($job), $this->repository->nextDelayed('::tube::'));
});
...
it('returns null if no delayed job exists', function () {
    ...
    assertNull($this->repository->nextDelayed('::tube::'));
});
...
it('fetches the stats for a tube', function () {
    ...
    assertEquals($expectedResponse, $this->repository->getStats($job));
});
...
it('fetches the job as well as the job stats', function (string $method) {
    ...
    $method = 'next' . $method;
    assertEquals(new Job($job, $stats), $this->repository->{$method}('::tube::', true));
})->with([
    ['Ready'],
    ['Delayed'],
    ['Buried'],
